...
if (request.getParameter("isPriceErr") != null) {
	isPriceErr = Boolean.valueOf(request.getParameter("isPriceErr"));
}
...
request.setCharacterEncoding("UTF-8");
try {
	int dispCategory = category.dispCategory();
%>
			...
			<div class="post">
				...
				<form action="<%=request.getContextPath()%>/fileupload"
					...
					<div class="information">
						...
						%>
						<%if (isPriceErr) {%>
						<p class="err-txt">正しい素材の価格を入力してください。</p>
						<%
						}
						%>
						<%if (isSuccessed) {%>
